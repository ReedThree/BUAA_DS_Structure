...
int main(void) {
    freopen("../article_random.txt", "r", stdin);
    struct HashTable *table = HashTable_createTable(100591);
    countWords(table);
    printResult(table);
...
unsigned long long HashTable_hash(struct HashTable_Key *data) {
    ...
    char *current = data->str;

    while (*current != '\0') {
        result = result * 127U + (unsigned long long)*current;
        current++;
    }

    return result;
}
void HashTable_destroyKey(struct HashTable_Key *target) { free(target); }
void HashTable_destroyValue(struct HashTable_Value *target) { free(target); }
